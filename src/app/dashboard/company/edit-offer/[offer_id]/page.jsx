...
async function fetchOffer(offer_id, setLoading) {
    try {
        const res = await fetch(`/api/offers/my-offers/offer/${offer_id}`);
...
export default function EditOfferPage() {
    ...
    useEffect(() => {
        if (!offer_id) return;
        fetchOffer(offer_id, setLoading).then((data) => {
    ...
    const editOffer = async (formData) => {
        setPosted(false);
        ...
        if (title && title !== offer.title) {
            ...
            res = await updateOffer.add(formdata);
        }

        if (location && location !== offer.location) {
            formdata.append("location", location);
            res = await updateOffer.add(formdata);
        }

        if (salary && salary !== offer.salary) {
            formdata.append("salary", salary);
            res = await updateOffer.add(formdata);
        }

        if (available_slots && available_slots !== offer.available_slots) {
            formdata.append("available_slots", available_slots);
            res = await updateOffer.update(formdata);
        }

        if (description && description !== offer.description) {
            formdata.append("description", description);
            res = await updateOffer.update(formdata);
        }

        if (expiration_date && expiration_date !== offer.expiration_date) {
        ...
        if (skills_arr.length > 0 && skills_arr !== offer.required_skills) {
            formdata.append("skills", JSON.stringify(skills_arr));
            res = await updateOffer.update(formdata);
        }

        if (languages_arr.length > 0 && languages_arr !== offer.languages) {
            formdata.append("languages", JSON.stringify(languages_arr));
            res = await updateOffer.update(formdata);
        }
    ...
    return (
        <>
            ...
            {loading === false &&
                <div className="min-h-screen m-0 py-12 px-0 overflow-y-auto">
                    <div className="max-w-[960px] mx-auto py-12 px-6">
                        ...
                        <div className="rounded-xl shadow-sm">
                            <form action={editOffer} className="p-8 space-y-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <input className="w-full h-12 rounded-lg border border-[#3a3f4a] bg-[#21242c] text-white outline-0 focus:ring-2 focus:ring-blue-500 transition-all px-4"
                                               defaultValue={offer?.title} name="title" placeholder="e.g. Senior UX Design Intern" type="text"/>
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <label className="text-white text-sm font-semibold">
                                            Location / City
                                        ...
                                        <div className="relative">
                                            ...
                                            <input className="w-full h-12 rounded-lg border border-[#3a3f4a] bg-[#21242c] text-white outline-0 focus:ring-2 focus:ring-blue-500 transition-all pl-10 pr-4"
                                                   defaultValue={offer?.location} name="location" type="text"/>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input className="w-full h-12 rounded-lg border border-[#3a3f4a] bg-[#21242c] text-white outline-0 focus:ring-2 focus:ring-blue-500 transition-all pl-8 pr-4"
                                                   defaultValue={offer?.salary} name="salary" placeholder="2500" min={0} type="number"/>
                                        </div>
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <label className="text-white text-sm font-semibold">
                                        ...
                                        <input className="w-full h-12 rounded-lg border border-[#3a3f4a] bg-[#21242c] text-white outline-0 focus:ring-2 focus:ring-blue-500 transition-all px-4"
                                               defaultValue={offer?.available_slots} name="available_slots" placeholder="3" min={0} type="number" />
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-2 pt-2">
                                    ...
                                    <textarea className="w-full min-h-[180px] rounded-lg border border-[#3a3f4a] bg-[#21242c] text-white outline-0 focus:ring-2 focus:ring-blue-500 transition-all p-4 resize-none"
                                              defaultValue={offer?.description} name="description" placeholder="Outline the core responsibilities, specific projects, and expected outcomes of this internship..." />
                                </div>
