...
export default function SupervisorForm() {
    ...
    const handleFetch = (query) => {
        ...
        fetchCompany(query).then((data) => setCompanies(data.data || []));
    }
    ...
    const handleSubmit = async (formData) => {
        ...
        const companyObj = companies.find(c => c.company_name === company);
        const company_id = companyObj?.company_id;
        ...
        if (!company_id) missing.company_id = true;

        if (missing.firstname || missing.lastname || missing.email || missing.password || missing.company_id) {
    ...
    return (
        <form className="flex flex-col gap-4" action={handleSubmit} onChange={handleChange}>
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="relative flex w-full items-center rounded-lg transition-all group focus-within:ring-2 focus-within:ring-primary/50 focus-within:border-primary">
                    <input
                        onChange={(e) => handleFetch(e.target.value)}
                        name="company_id"
                        list="companies"
                        className={`appearance-none w-full h-11 bg-blue-900/20 border rounded-lg pl-10 pr-3 text-sm text-blue-200 
                                         focus:outline-none focus:border-primary focus:ring-0 transition-colors ${
                            isMissing.company_id ? "border-red-800 placeholder:text-red-800" : "border-slate-200 placeholder:text-slate-600"
                        }`}
                        placeholder="Company..." required="" type="text" autoComplete="off" />
                    <datalist id="companies">
                        {
                            companies.length > 0 && companies.map(({company_id, company_name}) => {
                                return <option key={company_id} value={company_name} />
                        })
                        }
                    </datalist>
                    <div
                        className="absolute left-3 flex items-center justify-center pointer-events-none">
                            <span className="text-[20px]">
                                <Building2 className={isMissing.company_id ? "text-red-800" : "text-slate-500"} />
                            </span>
                    </div>
                </div>
            </div>
